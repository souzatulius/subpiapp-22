...
serve(async (req) => {
  ...
  try {
    ...
    if (tipo !== 'indicadores' && tipo !== 'relatorio' || !dados) {
...
Formato de resposta (em JSON):
{
  ...
  "fora_do_prazo": { "valor": "...", "comentario": "..." }
      ...
      // Implementação futura para geração de relatório descritivo
      // Apenas retorna os dados enviados por enquanto
    ...
    return new Response(JSON.stringify({ 
      error: error.message || "Ocorreu um erro ao gerar os insights",
      insights: null
    }), {
      status: 500,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
