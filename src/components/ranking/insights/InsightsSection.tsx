...
const InsightsSection: React.FC<InsightsSectionProps> = ({
  ...
  const gridClass = isMobileView ? "grid grid-cols-2 gap-4" : "grid grid-cols-4 gap-4";
  return <div>
      <h3 className="text-xl text-orange-800">Principais Insights</h3>
      <div className={gridClass}>
        {insights.map(insight => <Card key={insight.id} className="p-4 shadow-sm bg-white">
            ...
            <div className="flex justify-between items-center">
              ...
              {insight.trend === 'up' && <div className="flex items-center text-green-500">
                  <ChevronUp className="h-4 w-4 mr-1" />
                  <span className="text-xs">{insight.percentage}</span>
                </div>}
              {insight.trend === 'down' && <div className="flex items-center text-red-500">
                  <ChevronDown className="h-4 w-4 mr-1" />
                  <span className="text-xs">{insight.percentage}</span>
                </div>}
              {insight.count && <span className="text-xs text-gray-500">{insight.count}</span>}
            </div>
          </Card>)}
      </div>
    </div>;
