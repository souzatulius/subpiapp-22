...
const InsightCard: React.FC<InsightCardProps> = ({
  ...
  return (
    ...
    >
      <Card className={`overflow-hidden rounded-3xl shadow-sm hover:shadow-md transition-all ${isSimulated ? 'border-orange-300 bg-orange-50' : 'border-blue-100'} relative`}>
          ...
          {isLoading ? (
            <div className="mt-2 space-y-2">
              <div className="flex items-center">
                <Skeleton className="h-8 w-24 bg-blue-50 rounded-full" />
                <Loader2 size={16} className="ml-2 animate-spin text-blue-500" />
              </div>
              <Skeleton className="h-4 w-full mt-2 bg-blue-50 rounded-full" />
            </div>
          ) : (
