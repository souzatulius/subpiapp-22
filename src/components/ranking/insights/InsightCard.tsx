...
const InsightCard: React.FC<InsightCardProps> = ({
  ...
  const formatPriorityDisplay = (text: string): string => {
    if (title.toLowerCase() === 'prioridade' && text.toLowerCase() === 'alta') {
      return 'Alta';
    }
    return text;
  };
              ...
              {showAnalysis && analysis ? (
                ...
                >
                  <p className={`text-lg font-bold mt-1 text-orange-500`}>
                    {formatPriorityDisplay(isSimulated ? formatValue(value) : value)}
                  </p>
