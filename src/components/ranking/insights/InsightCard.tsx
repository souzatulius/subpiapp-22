      ...
      <Card className="rounded-2xl bg-zinc-200 px-[23px] mx-[3px]">
        ...
        <AnimatePresence>
          ...
          duration: 0.2
              ...
              {analysis && <button onClick={() => setShowAnalysis(!showAnalysis)} className="p-1 rounded-full bg-gray-100 hover:bg-gray-200 text-gray-600 transition-colors" title={showAnalysis ? "Mostrar valores" : "Ver análise interpretativa"}>
                  <Search size={12} />
                </button>}
              
              <button onClick={e => e.stopPropagation()} className="p-1 rounded-full bg-gray-100 hover:bg-gray-200 text-gray-600 transition-colors" title="Ocultar este card">
                <EyeOff size={12} />
              </button>
            </motion.div>}
        </AnimatePresence>
        ...
        <div className="p-2 rounded-2xl px-0 mx-0">
          <div className="flex items-center justify-between">
            ...
            {trend && <div className={`flex items-center ${trend === 'up' ? 'text-green-500' : trend === 'down' ? 'text-red-500' : 'text-gray-400'}`}>
                {trend === 'up' ? <TrendingUp size={14} className="animate-pulse" /> : trend === 'down' ? <TrendingDown size={14} className="animate-pulse" /> : <Minus size={14} />}
              </div>}
          </div>
          
          {isLoading ? <div className="mt-1 space-y-1">
