...
export const ProfileMenu: React.FC = () => {
  ...
  return <Popover open={isProfileOpen} onOpenChange={setIsProfileOpen}>
      <PopoverTrigger asChild>
        <Button variant="ghost" size="icon" className="bg-gray-100 hover:bg-gray-200 rounded-full">
          {userProfile?.foto_perfil_url ? <Avatar className="h-8 w-8">
            ...
            </Avatar> : <User className="h-5 w-5 text-[#003570]" />}
        </Button>
      </PopoverTrigger>
      <PopoverContent className="w-64 p-0 overflow-hidden">
        <div className="bg-gray-100 p-4 px-0 rounded-none">
          <div className="flex items-center space-x-3 bg-transparent my-0">
            <Avatar className="h-12 w-12 text-gray-900">
              {userProfile?.foto_perfil_url ? <AvatarImage src={userProfile.foto_perfil_url} alt={userProfile.nome_completo} /> : <AvatarFallback className="text-orange-600 bg-gray-900">{userProfile?.nome_completo ? getUserInitials(userProfile.nome_completo) : 'U'}</AvatarFallback>}
            </Avatar>
            <div>
        ...
        <div className="p-1 bg-zinc-50">
          <Button variant="ghost" className="w-full justify-start text-left py-2 hover:bg-[#f57c35] hover:text-white transition-colors" onClick={() => {
          setIsProfileOpen(false);
          navigate('/settings');
        }}>
          ...
          <Button variant="ghost" className="w-full justify-start text-left py-2 hover:bg-[#f57c35] hover:text-white transition-colors" onClick={handleLogout}>
