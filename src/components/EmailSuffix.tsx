...
const EmailSuffix: React.FC<EmailSuffixProps> = ({
  ...
  return <div className={`flex w-full rounded-lg overflow-hidden border ${borderColor} transition-colors ${className}`} onClick={() => {
    ...
    inputRef.current?.focus();
      ...
      <div className="w-full px-4 py-2 border border-gray-300 rounded-r-xl focus:outline-none focus:ring-2 focus:ring-[#003570] focus:border-transparent transition-all duration-200 pr-1">
        {suffix}
      </div>
    </div>;
};
