...
const AprovarNotaForm: React.FC<AprovarNotaFormProps> = ({
  ...
  const handleAprovarRejeitar = async (status: 'aprovado' | 'rejeitado') => {
    ...
    try {
      ...
      const {
        error
      } = await supabase.from('notas_oficiais').update({
        status,
      ...
      toast({
        ...
        description: status === 'aprovado' ? "A nota oficial foi aprovada e está pronta para publicação." : "A nota oficial foi rejeitada."
      });
