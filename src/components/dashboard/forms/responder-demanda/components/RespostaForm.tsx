  ...
  return <div className="space-y-4 animate-fade-in">
      ...
      <Card className="border shadow-sm">
        <CardContent className="p-4 space-y-4">
          ...
          {selectedDemanda.detalhes_solicitacao && <div className="space-y-2">
          ...
          {selectedDemanda.perguntas && <div className="space-y-2">
              <h3 className="font-semibold text-subpi-blue flex items-center gap-2">
                <HelpCircle className="h-5 w-5" /> Perguntas e Respostas
              </h3>
              <QuestionsAnswersSection perguntas={selectedDemanda.perguntas} resposta={resposta} onRespostaChange={onRespostaChange} />
            </div>}

          {(selectedDemanda.arquivo_url || selectedDemanda.anexos?.length > 0) && <div>
              <h3 className="font-semibold text-subpi-blue flex items-center gap-2">
                <Paperclip className="h-5 w-5" /> Anexos
              </h3>
              <AttachmentsSection arquivo_url={selectedDemanda.arquivo_url} anexos={selectedDemanda.anexos} onViewAttachment={url => window.open(url, '_blank')} onDownloadAttachment={url => window.open(url, '_blank')} />
            </div>}
          ...
          <div>
            ...
            <CommentsSection comentarios={comentarios} onChange={setComentarios ? value => setComentarios(value) : () => {}} placeholder="Tem mais alguma informação?" simplifiedText={true} />
          </div>
        </CardContent>

        <CardFooter className="border-t bg-white sticky bottom-0">
          <FormFooter isLoading={isLoading} onSubmit={handleSubmitWithExtra} allQuestionsAnswered={allQuestionsAnswered()} />
        </CardFooter>
      </Card>
    </div>;
};
