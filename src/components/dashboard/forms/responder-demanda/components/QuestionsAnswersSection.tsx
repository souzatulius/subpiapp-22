...
const QuestionsAnswersSection: React.FC<QuestionsAnswersSectionProps> = ({
  ...
  if (!perguntas || normalizedQuestions.length === 0) {
    return <Card className="bg-gray-50 p-6 rounded-xl text-center shadow-sm border border-gray-200">
  ...
  return <div className="space-y-5">
      ...
      <div className="space-y-5">
        {normalizedQuestions.map((pergunta: string, index: number) => <Card key={`question-${index}`} className="overflow-hidden border-orange-200 hover:shadow-md transition-all duration-300 rounded-xl">
            <CardContent className="p-0">
              <div className="p-4 text-white rounded-t-xl bg-orange-400">
                <p className="text-base font-semibold">{pergunta}</p>
              </div>
              <div className="p-4">
                <Textarea id={`resposta-${index}`} placeholder="Digite sua resposta" className="min-h-[120px] w-full border border-gray-300 focus:border-blue-400 focus:ring-blue-300 rounded-lg" value={resposta[index.toString()] || ''} onChange={e => onRespostaChange(index.toString(), e.target.value)} />
              </div>
            </CardContent>
          </Card>)}
      </div>
