...
const PriorityDeadlineStep: React.FC<PriorityDeadlineStepProps> = ({
  ...
  return <div className="space-y-4">
      <div>
        <Label htmlFor="prioridade" className={`block mb-2 ${hasError('prioridade') ? 'text-orange-500 font-semibold' : ''}`}>
          Prioridade
        </Label>
        
        <div className="mb-4">
          {/* Horizontal Traffic Light */}
          <div className="flex items-center p-3 rounded-lg w-64 h-24 mb-2 bg-transparent">
            {/* Red Light */}
            <button onClick={() => handleSelectChange('prioridade', 'alta')} className={`w-16 h-16 rounded-full mx-2 transition-all ${formData.prioridade === 'alta' ? 'ring-4 ring-gray-400' : ''}`} style={{
            backgroundColor: '#FF4136'
          }} aria-label="Prioridade Alta" />
            {/* Yellow Light */}
            <button onClick={() => handleSelectChange('prioridade', 'media')} className={`w-16 h-16 rounded-full mx-2 transition-all ${formData.prioridade === 'media' ? 'ring-4 ring-gray-400' : ''}`} style={{
            backgroundColor: '#FFDC00'
          }} aria-label="Prioridade Média" />
            {/* Green Light */}
            <button onClick={() => handleSelectChange('prioridade', 'baixa')} className={`w-16 h-16 rounded-full mx-2 transition-all ${formData.prioridade === 'baixa' ? 'ring-4 ring-gray-400' : ''}`} style={{
          ...
          <div className="w-48">
            <Popover>
              <PopoverTrigger asChild>
                <Button variant="outline" className={cn("w-full justify-start text-left font-normal", !formData.prazo_resposta && "text-muted-foreground")}>
                  <CalendarIcon className="mr-2 h-6 w-6" />
                  {formData.prazo_resposta ? format(new Date(formData.prazo_resposta), "dd/MM/yyyy") : <span>Selecione uma data</span>}
                </Button>
              </PopoverTrigger>
