...
import { LayoutDashboard, Droplet, Trash2, Trees, AlertTriangle, MessageSquare, Briefcase, Book, Users, Mail, Heart, Home, Code, Lightbulb } from 'lucide-react';
  ...
  return <div className="space-y-4">
      ...
      <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
          {areasCoord.map(area => <Button key={area.id} type="button" variant={formData.area_coordenacao_id === area.id ? "default" : "outline"} className={`h-auto py-3 flex flex-col items-center justify-center gap-2 ${formData.area_coordenacao_id === area.id ? "ring-2 ring-[#003570]" : ""} ${hasError('area_coordenacao_id') ? 'border-orange-500' : ''}`} onClick={() => handleSelectChange('area_coordenacao_id', area.id)}>
      ...
      {formData.area_coordenacao_id && <div className="animate-fadeIn">
          <Label htmlFor="servico_id" className={`block ${hasError('servico_id') ? 'text-orange-500 font-semibold' : ''}`}>
            Serviço {hasError('servico_id') && <span className="text-orange-500">*</span>}
          </Label>
          <div className="relative">
            <Input type="text" name="serviceSearch" value={serviceSearch} onChange={handleChange} className={`w-full rounded-lg ${hasError('servico_id') ? 'border-orange-500 ring-orange-500' : ''}`} placeholder="Pesquisar serviço" />
            ...
            {serviceSearch && filteredServicesBySearch.length > 0 && <div className="absolute z-10 w-full mt-1 bg-white border border-gray-200 rounded-lg shadow-lg max-h-60 overflow-y-auto">
                {filteredServicesBySearch.map(service => <div key={service.id} onClick={() => handleServiceSelect(service.id)} className="px-4 py-2 cursor-pointer bg-gray-100">
                    {service.descricao}
                  </div>)}
              </div>}
          </div>
          ...
          {formData.servico_id ? <div className="mt-2 p-2 bg-blue-50 rounded-lg text-sm">
            ...
            </div> : hasError('servico_id') && <p className="text-orange-500 text-sm mt-1">{getErrorMessage('servico_id')}</p>}
        </div>}
    </div>;
};
