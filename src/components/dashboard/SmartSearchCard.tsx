...
const SmartSearchCard: React.FC<SmartSearchCardProps> = ({
  ...
  const suggestions = React.useMemo(() => {
    ...
    const matchedRoutes = Object.entries(keywordRoutes).filter(([keyword]) => keyword.toLowerCase().includes(lowerQuery)).map(([keyword, route]) => ({
  ...
  return <form onSubmit={handleSubmit} className="w-full h-full">
      <div className="relative w-full h-full">
        <Search className="absolute left-4 top-1/2 transform -translate-y-1/2 h-6 w-6 text-orange-500 z-10" />
        <Input ref={inputRef} type="text" placeholder={placeholder} value={query} onChange={e => setQuery(e.target.value)} onFocus={() => setShowSuggestions(true)} onBlur={() => setTimeout(() => setShowSuggestions(false), 200)} className="pl-14 pr-4 py-6 rounded-xl border border-gray-300 w-full h-full bg-white text-2xl text-gray-800 placeholder:text-gray-600" />
      </div>
      ...
      <AnimatePresence>
        ...
        duration: 0.2
            ...
            <ul className="py-1">
              {suggestions.map((suggestion, i) => <li key={i} className="px-3 py-2 hover:bg-gray-100 cursor-pointer flex items-center text-xl font-medium" onMouseDown={() => handleSelectSuggestion(suggestion)}>
