...
const LeftContentSection: React.FC = () => {
  ...
  useEffect(() => {
    // Initialize fitty on all three title lines with larger max size
    if (titleLine1Ref.current && titleLine2Ref.current && titleLine3Ref.current) {
      const fittyInstance1 = fitty(titleLine1Ref.current, {
        minSize: 24,
        maxSize: 160, // Increased max size substantially to allow for text-9xl or text-10xl equivalent
        multiLine: false
      });
      
      const fittyInstance2 = fitty(titleLine2Ref.current, {
        minSize: 24,
        maxSize: 160, // Increased max size substantially
        multiLine: false
      });
