...
import { useComunicacaoCriacao } from './hooks/useComunicacaoCriacao';
import { useAuth } from '@/contexts/AuthContext';
import { Button } from '@/components/ui/button';
import { Card } from '@/components/ui/card';
import { ArrowLeft } from 'lucide-react';
import DemandaInfoCard from './components/DemandaInfoCard';
import CriarComunicacaoForm from './components/CriarComunicacaoForm';
...
const DetalhesDemanda: React.FC<DetalhesDemandaProps> = ({ demandaId, onClose }) => {
  const { demanda, loading, error, comunicacaoExistente, checkingComunicacao } = useDemandaDetalhes(demandaId);
  const { user } = useAuth();
  const { 
    titulo, 
    setTitulo, 
    ...
    gerarSugestao,
    isGerandoSugestao
  } = useComunicacaoCriacao(demandaId, demanda, onClose);
        ...
        <div className="lg:col-span-2">
          <Card className="p-6">
            <CriarComunicacaoForm 
              ...
              isPending={isPending}
              comunicacaoExistente={comunicacaoExistente}
              isCheckingComunicacao={checkingComunicacao}
              demanda={demanda}
