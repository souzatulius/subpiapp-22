...
const ComunicacaoDashboard: React.FC<ComunicacaoDashboardProps> = ({
  ...
  const renderSpecialCardContent = useCallback((cardId: string) => {
    ...
    if (cardId === 'origem-demandas-card' || cardId.includes('origem-demandas') || cardId.includes('origemDemandas') || cardId.includes('origin-demand-chart')) {
      return <OriginsDemandChartCompact className="w-full h-full" color={foundCard?.color} title={foundCard?.title} subtitle={foundCard?.subtitle} />;
    }
    if (cardId === 'press-request-card' || cardId.includes('press-request')) {
      return <PressRequestCard />;
    }
    if (cardId === 'acoes-pendentes-card' || cardId.includes('acoes-pendentes') || foundCard && foundCard.isPendingTasks || foundCard && foundCard.type === 'pending_tasks') {
      return <PendingTasksCard id={cardId} title={foundCard?.title || 'Ações Pendentes'} userDepartmentId={specialCardsData.coordenacaoId} isComunicacao={true} />;
    }
    return null;
  ...
  return <div className="space-y-6 bg-[#FFFAFA]">
      <div className="w-full mb-2">
        <WelcomeCard title="Comunicação" description="Gerencie demandas e notas oficiais" icon={<MessageSquareReply className="h-8 w-8 mr-2 text-gray-600" />} color="bg-gradient-to-r from-[#0066FF] to-blue-700" showResetButton={true} resetButtonIcon={<RotateCcw className="h-4 w-4" />} onResetClick={handleResetDashboard} />
      </div>
      
      {/* Save status indicator - only render if there's save information to show */}
      {!isPreview && user && (isSaving || lastSaved || hasUnsavedChanges) && <div className="flex items-center justify-between px-4 py-2 bg-white rounded-xl shadow-sm">
          <div className="text-sm text-gray-600">
            {isSaving ? <span className="flex items-center">
          ...
          {hasUnsavedChanges && <Button size="sm" className="bg-blue-600 hover:bg-blue-700 text-white rounded-xl" onClick={handleManualSave}>
      ...
      {isLoading ? <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          {Array.from({
        length: 8
      }).map((_, index) => <Skeleton key={index} className="h-32 w-full rounded-xl" />)}
        </div> : processedCards.length > 0 ? <div className="px-2 py-2">
            <CardGridContainer cards={processedCards} onCardsChange={handleCardsReorder} onEditCard={handleCardEdit} onHideCard={handleCardHide} isMobileView={isMobile} isEditMode={isEditMode} specialCardsData={specialCardsData} disableWiggleEffect={true} showSpecialFeatures={true} renderSpecialCardContent={renderSpecialCardContent} />
          </div> : <div className="p-4 text-center text-gray-500">
            Nenhum card disponível.
          </div>}
      
      {selectedCard && <EditCardModal isOpen={isEditModalOpen} onClose={() => setIsEditModalOpen(false)} onSave={handleSaveCardEdit} card={selectedCard} />}
