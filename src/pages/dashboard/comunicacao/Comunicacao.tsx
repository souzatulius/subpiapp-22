...
const ComunicacaoDashboard: React.FC<ComunicacaoDashboardProps> = ({
  ...
  const processedCards = useMemo(() => {
    ...
    return cards.filter(card => !card.isHidden).map(card => {
  ...
  return <div className="space-y-6 bg-[#FFFAFA]">
      <div className="w-full mb-4">
        <WelcomeCard title="Comunicação" description="Gerencie demandas e notas oficiais" icon={<MessageSquareReply className="flex items-start gap-3 h-20 w-20" />} color="bg-gradient-to-r from-[#0066FF] to-blue-700" showResetButton={false} className="text-blue-300 h-15 w-15 // text-2xl font-bold mb-2 text-blue-500 py-0 my-[4px] " />
      </div>
      ...
      {isLoading ? <div className="grid grid-cols-4 gap-6">
          {[...Array(8)].map((_, i) => <div key={i} className="col-span-1">
              <Skeleton className="h-40 w-full" />
            </div>)}
        </div> : <>
          <CardGridContainer cards={processedCards} onCardsChange={handleCardsReorder} onEditCard={handleCardEdit} onHideCard={handleCardHide} isEditMode={isEditMode} isMobileView={isMobile} renderSpecialCardContent={renderSpecialCardContent} />
